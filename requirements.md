...
| 行送り | `\` | 理由 |
|--------|-----|------|
| リスト項目 → リスト項目 | 付けない | 行送りだけで別項目になる。付けると各項目末尾に `<br>` が入る |
| 空行の前後 | 付けない | 段落の区切り。付けると `\` だけの行になる |
| それ以外 | 付ける | 付けないとソフト改行（＝空白）に潰れる |

リストとみなすのは**セルの先頭か空行の後から始まる並び**だけ。pandoc の markdown は
リストで段落を中断しない（`lists_without_preceding_blankline` は既定で無効）ため、
`前置き` の直後の `- 項目` はリストにならず、ここで `\` を落とすと改行が消える。

読み込み時は `<br>` / `<br/>` / 行末 `\` に加えて、上表で `\` を付けない行送り
（リスト項目どうし・空行の前後）も改行として戻す。セル内の空行は段落区切りとして保つ
（前後の空行はセルを縦に埋める余白なので落とす）。

> **検証済み（Quarto 同梱 pandoc ＋ `design-doc.lua`）**
>
> `\` 無しのリスト行は `<ul>` / `<ol>` になり、`\` を付けると各項目末尾に `<br>` が入る。
> 段落＋空行＋リストは `<p>` とリストに分かれる。結合を展開した**素のグリッド表**でも
> `merge-cols` は効く（`is_plain_grid` ガードを通るため）。
